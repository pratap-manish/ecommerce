...
function Cart(props) {
    ...
    return (
        ...
        <div className='mx-auto row justify-center mt-2 bg-maintexttheme mx-0 mx-sm-2'>
            {cartProducts.map((product)=>{
             cartTotal += parseFloat((product.price * product.count).toFixed(2))
                return <div className="p-0 p-sm-1 col-12 col-md-4 col-lg-3">
                <div
              value={product.id}
            ...
            <Link to={"/details/" + product.title}  className="d-flex flex-column text-center justify-content-around h-100 text-decoration-none p-3" onClick={()=>{props.setProductId(product.id )}}  >
              ...
              <button className='btn btn-outline-maintexttheme d-flex mx-1' onClick={()=>decreaseQuantity(product)}><span class="material-symbols-outlined">remove</span></button>
              <span>Quantity {product.count}</span>
              <button className='btn btn-maintexttheme d-flex mx-1' onClick={()=>increaseQuantity(product)}><span class="material-symbols-outlined">add</span></button>
              </div>
            </div>
            </div>
            })
          }
          <div className='bg-maintexttheme p-0 p-sm-1'>
            <div className='d-flex justify-content-around flex-column text-center mb-4 bg-white rounded-3 p-2'>
          <h4 className='mt-4'> {parseFloat(cartTotal.toFixed(2))>0?"Cart Total $" + parseFloat(cartTotal.toFixed(2)):"Cart is Empty"}</h4>
          {parseFloat(cartTotal.toFixed(2))>0 && <button className='btn btn-maintexttheme mx-auto' style={{width:"calc(10rem + 10vw)"}}>Continue to Buy</button>}
          </div>
          </div>
        </div>
        </>
